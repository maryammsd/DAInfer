...
DAInfer+ provides several capabilities for inferring both data-flow and alias specifications for Android and Java APIs using LLMs or embedding models. You can choose any of the following options to run your desired capability through the command line:
...
```
You can configure your desired LLM model and embedding model in the `config.py` by modifying the variables `LLM` and `EMBEDDING_MODEL`. Currently, our tool only supports E5 and SentenceBert models as well as GPT, Qwen, DeppSeek Coder, and DeepSeek V2. We recently added the functions to be able to prompt Codex and Claude, and you can use them. We used `ollama` and locally built the open-sourced LLM models on our machine. You need to do the same to use these models. To use GPT, you only need to set the OpenAI key in the variable `openai_key_baseline` in the `config.py` file. 
...
ATTENTION: The eager mode will invoke the OpenAI API for a large number of methods documented in the library, which may incur significant costs. We recommend using lazy mode to avoid the high cost.
...
The directory `oracle` contains three sources of oracles: those specified in FlowDroid and USpec, and those specified manually. The generated Atlas models are stored in `baseline/atlas/models`. We referred to all the other oracles when we manually specified ours, which is stored in the directory `oracle/ManualOracle` for either alias or data-flow inference.
...
In each directory `analysisType_model_FourTypes_m_n_temp1_temp2`, the three sub-directories contain the following (intermediate) results:

- The directory `methodInfo`: contains the JSON files that record the method type signatures and the results of memory operation abstraction, either by the LLM (e.g., methodRetParaDic.json and typeConsistentSpecs.json) or embedding models (e.g., under the attribute `dataflowResult` in the file dataflow_result_prompts.json). For our recent contribution, responses to prompts generated by recent LLMs are also stored in this directory (e.g., under attribute `response` in the file dataflow_result_prompts.json).

- The directory `methodPrompt` contains the prompting results of the first stage of the two-staged prompting, i.e., the typical verbs indicating the four kinds of memory operations.

- The directory `inferResult`: The results of the API data flow or aliasing specification inference. The JSON file `benchmark_CHADic.json` maintains the class hierarchy relation. It can contain different JSON files based on the analysis type:
  - If we use two-stage prompting to infer Alias API specifications, the JSON file `benchmark_inferredSpecs.json` contains all the inferred specifications. The JSON file `benchmark_NNSet.json` contains the identified semantic units, namely named entities in the method names. The JSON file `benchmark_retArgSpecCandidate.json` stores all API aliasing specifications that satisfy the degree and validity constraints, but may not satisfy the semantic unit and memory operation constraints. The 
  -  If we use embedding models to infer Alias API specifications, the JSON file `benchmark_inferredSpecs.json` contains all the inferred specifications. The JSON file `benchmark_NNSet.json` contains the identified semantic units, namely named entities in the method names. The JSON file `benchmark_retArgSpecCandidate.json` stores all API aliasing specifications that satisfy the degree and validity constraints, but may not satisfy the semantic unit and memory operation constraints. Detailed information, such as the number of tokens, runtime, etc., is logged in the text file `benchmark_embedding_usage.txt` in this directory.
  - If we use one-stage prompting with recent advanced LLMs to infer data flow API specifications, the JSON file `benchmark_inferredDataflowSpecs.json` contains all the inferred data flow summaries for API methods. Detailed information, such as the number of tokens, runtime, etc., is logged in the text file `benchmark_LLM_token_usage.txt` in this directory.
  - If we use embedding models to infer data flow API specifications, the JSON file `benchmark_inferredSourceSinkSpecs.json` contains all the inferred data flow summaries for API methods. Detailed information, such as the number of tokens, runtime, etc., is logged in the text file `benchmark_embedding_usage.txt` in this directory.
...
In addition to the above baselines, we used StubDroid as the baseline for comparing our tool's accuracy, precision, and performance in data flow extraction. We keep the summaries generated by StubDroid for our target classes in the `baseline\stubdroid\stubdroidsummaries` folder, which are produced during a separate analysis of the Android framework source code. We used these results to compare our tool with StubDroid based on the collected oracle set.

## Client Analysis
We recently added some experiments to assess the impact of DAInfer+ in taint-analysis. We added our code and the readme to work with them under data/client-analysis folder. 
